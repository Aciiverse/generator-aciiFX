...
router.post("/register", validateRegister, async (req, res) => {
    ...
    try {
        const users: { uuid: UsersTable["uuid"] }[] & ResultSetHeader = await db.query("SELECT uuid FROM users WHERE LOWER(username) = LOWER(?)", [
            body.username,
        ]);
        ...
        // create new user
        await db.query("INSERT INTO users (uuid, username, email, password, registered, verified) VALUES (?, ?, ?, ?, now(), false);", [
            uuid.v4(),
            body.email,
            body.username,
            hash,
